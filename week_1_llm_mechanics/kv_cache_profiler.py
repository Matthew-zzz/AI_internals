...
def profile_kv_cache(model, context_length: list[int] = [512, 1024, 2048, 8192], generate_tokens_count: int = 30):
  ...
  for length in context_length:
    ...
    logger.info(f"Контекстное окно из {length - generate_tokens_count if length == max_pos else length} токенов")

    input_ids = torch.randint(
      low=0, high=model.config.vocab_size, size=(1, (length - generate_tokens_count) if length == max_pos else length )
    ).to(device)
    ...
    speedup_factor = (
        without_kv_cache["elapsed"] /
        with_kv_cache["elapsed"]
    )
    logger.success(f"Коэффициент ускорения: {speedup_factor:.2f}x")


def check_time_kv_cache(model, input_ids: torch.Tensor, use_cache: bool = True, generate_tokens_count: int = 30):
