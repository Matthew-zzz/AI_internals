...
def func_top_p(tensor: torch.Tensor, top_p: float = 0.95):
    ...
    return_tensor_indices = masked_probs.scatter(
        dim=-1, index=sorted_indices, src=masked_probs
    )
...
def sample_text_token(
    ...
    # Детерменированное сэмплирование
...
def generate_text_custom(prompt, max_new_tokens, temperature):
    ...
    llm_answer = ""
    input_ids = (
        tokenizer((prompt + llm_answer), return_tensors="pt").to(device).input_ids
    )
